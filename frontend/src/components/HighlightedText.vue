...
const segments = computed(() => {
  ...
  return result.map(entry => ({ txt: text.substring(...entry.range), highlighted: entry.highlighted }))
})
</script>

<template>
  <b class="has-text-weight-semibold">
    <span v-for="(segment, index) of segments" :key="index" :class="{ 'has-text-weight-bold is-underlined': segment.highlighted }">
      {{ segment.txt }}
    </span>
  </b>
</template>
